			...
			for (int j = 1; j < n; j++)
				if (nums[j].size() <= i)
				{
					...
					for (int in = 0; in < nums[j].size(); in++)
					{
						...
						dp[0][j][i] = max(dp[0][j][i], 1 + max(dp[maxlog][nums[j][in]][i - in - 1], dp[maxlog][bef - (1 << maxlog) + 1][i - in - 1]));
						bef = nums[j][in] - 1;
					}
					if (bef != -1)
					{
						int maxlog = log2(bef + 1);
						dp[0][j][i] = max(dp[0][j][i], 1 + max(dp[maxlog][0][i - nums[j].size()], dp[maxlog][bef - (1 << maxlog) + 1][i - nums[j].size()]));
					}
				}
			for (int j = 1; j < logm; j++)
				for (int l = 0; l < n; l++)
					dp[j][l][i] = max(dp[j - 1][l][i], dp[j - 1][min(n - 1, l + (1 << (j - 1)))][i]);
		}
