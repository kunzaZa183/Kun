...
int k, x, i, j;
void dcdp(int left, int right, int mini, int maxi)
{

    int mid = (mini + maxi) / 2, bestop = -1;
    for (int y = mini; y <= maxi; y++)
    {
        int val = dp[k - 1][x + 1][y - 1] + (x - i) * (qsum[x] - qsum[i - 1]) + (j - y) * (qsum[j] - qsum[y - 1]) + 2 * (x - i + 1) * (j - y + 1);
        if (val < dp[k][i][j])
        {
            dp[k][i][j] = val;
            bestop = y;
